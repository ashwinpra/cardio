...
export default function Lobby() {
  ...
  const hostId = gameState.players.length > 0 ? gameState.players[0].id : null;
  ...
  const cleanGameString = (gameType: string) => {
    return gameType.split('_').map(word => word.charAt(0) + word.slice(1).toLowerCase()).join(' ');
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10 max-w-7xl mx-auto w-full">
        <div>
          ...
          <div className="flex items-center gap-2.5">
            <span className="text-[10px] md:text-xs text-outline uppercase tracking-widest font-label-md">Session Code</span>
        ...
        <div className="w-full sm:w-auto text-left sm:text-right flex flex-row-reverse sm:flex-col items-center sm:items-end justify-between sm:justify-end gap-2">
          ...
          <div>
            <p className="text-[10px] md:text-xs text-outline uppercase tracking-widest font-label-md">Participants</p>
        ...
        <div className={`grid ${isLiterature || isSpades ? 'md:grid-cols-3' : 'md:grid-cols-2'} gap-6`}>
          ...
          {(isLiterature || isSpades) && (
            <div className="rounded-3xl p-6 bg-surface-container-lowest shadow-sm border border-outline-variant">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="text-xs font-label-md text-primary uppercase tracking-wider">Team A</h3>
              </div>
              <div className="space-y-3">
                {teamA.map(p => (
                  <PlayerCard key={p.id} p={p} isMe={p.id === localId || p.id === myPlayerId} isHost={p.id === hostId} color="primary" />
                ))}
          ...
          <div className="rounded-[32px] p-6 flex flex-col bg-surface-container-lowest border border-outline-variant shadow-md relative overflow-hidden order-first md:order-none">
            <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
              <span className="material-symbols-outlined text-6xl md:text-8xl">sports_esports</span>
            </div>

            {!isJoined ? (
              <>
                <h3 className="font-headline-sm text-on-surface mb-2">Claim Your Spot</h3>
                <p className="text-sm font-body-md text-on-surface-variant mb-8">Enter your alias to join the session.</p>

                <div className="space-y-6 flex-1">
                  <div>
                    <label className="block text-[10px] font-label-md text-on-surface-variant uppercase tracking-widest mb-2">Your Name</label>
                  ...
                  {(isLiterature || isSpades) && (
                    <div>
                      <label className="block text-[10px] font-label-md text-on-surface-variant uppercase tracking-widest mb-2">Choose Side</label>
                      <div className="grid grid-cols-2 gap-3">
                        <button onClick={() => setSelectedTeam('TEAM_A')}
                          className={`p-4 rounded-2xl font-label-md text-sm transition-all border-2 ${selectedTeam === 'TEAM_A' ? 'bg-primary-container/20 text-primary border-primary' : 'bg-surface-container text-on-surface-variant border-transparent hover:bg-surface-container-high'
                            }`}>Team A</button>
                        <button onClick={() => setSelectedTeam('TEAM_B')}
                          className={`p-4 rounded-2xl font-label-md text-sm transition-all border-2 ${selectedTeam === 'TEAM_B' ? 'bg-secondary-container/20 text-secondary border-secondary' : 'bg-surface-container text-on-surface-variant border-transparent hover:bg-surface-container-high'
                            }`}>Team B</button>
                      </div>
                    </div>
                  )}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center py-4">
                ...
                <h3 className="font-headline-sm text-on-surface mb-2">Ready to Play</h3>
                <p className="text-sm font-body-md text-on-surface-variant mb-10 max-w-[200px]">
                  {isHost ? 'You are the host. Start when ready.' : 'Waiting for the host to initiate the game.'}
                </p>
          ...
          {!(isLiterature || isSpades) && (
            <div className="rounded-3xl p-6 bg-surface-container-lowest shadow-sm border border-outline-variant">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="text-xs font-label-md text-secondary uppercase tracking-wider">Players</h3>
              </div>
              <div className="space-y-3">
                {gameState.players.map(p => (
                  <PlayerCard key={p.id} p={p} isMe={p.id === localId || p.id === myPlayerId} isHost={p.id === hostId} color="secondary" />
                ))}
          ...
          {(isLiterature || isSpades) && (
            <div className="rounded-3xl p-6 bg-surface-container-lowest shadow-sm border border-outline-variant">
              <div className="flex items-center gap-2 mb-6">
                <div className="w-2.5 h-2.5 rounded-full bg-secondary" />
                <h3 className="text-xs font-label-md text-secondary uppercase tracking-wider">Team B</h3>
              </div>
              <div className="space-y-3">
                {teamB.map(p => (
                  <PlayerCard key={p.id} p={p} isMe={p.id === localId || p.id === myPlayerId} isHost={p.id === hostId} color="secondary" />
...
function PlayerCard({ p, isMe, isHost, color }: { p: any, isMe: boolean, isHost: boolean, color: 'primary' | 'secondary' }) {
  const bg = color === 'primary' ? 'bg-primary-container/20' : 'bg-secondary-container/20';
  const text = color === 'primary' ? 'text-primary' : 'text-secondary';
  const iconBg = color === 'primary' ? 'bg-primary-container/40' : 'bg-secondary-container/40';
  const isDisconnected = p.isConnected === false;

  return (
    <div className={`flex items-center gap-4 px-4 py-3 rounded-2xl ${bg} border border-transparent hover:border-outline-variant transition-all ${isDisconnected ? 'opacity-50' : ''}`}>
      <div className={`w-8 h-8 rounded-xl ${iconBg} flex items-center justify-center ${text} font-headline-sm text-sm`}>
        {p.name.charAt(0).toUpperCase()}
      </div>
      <span className={`text-sm font-label-md text-on-surface ${isDisconnected ? 'line-through' : ''}`}>{p.name}</span>
      <div className="ml-auto flex items-center gap-2">
        {isDisconnected && (
          <span className="text-[9px] font-label-md text-error uppercase tracking-widest">offline</span>
        )}
        {isHost && (
          <span className="text-[9px] font-label-md text-primary uppercase tracking-widest bg-primary-container/30 px-2 py-0.5 rounded-full">host</span>
        )}
        {isMe && <span className={`${text} text-[10px] font-label-md uppercase tracking-widest`}>You</span>}
      </div>
    </div>
  );
}
...
function EmptySlot() {
  return <div className="h-[52px] rounded-2xl border-2 border-dashed border-outline-variant flex items-center justify-center text-[10px] font-label-md text-outline uppercase tracking-widest">Waiting for player...</div>;
}
